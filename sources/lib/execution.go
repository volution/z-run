...
func prepareExecution (_libraryUrl string, _libraryFingerprint string, _interpreter string, _scriptlet *Scriptlet, _includeArguments bool, _context *Context) (*exec.Cmd, []int, *Error) {
	...
	if len (_scriptlet.Body) < (1 * 1024 * 1024 - 128 * 1024) {
		...
		if _context.cacheRoot == "" {
			// FIXME:  We should make sure that the cache path is never empty!
			panic (0xd6f17610)
		}
		_temporaryPath := path.Join (_context.cacheRoot, generateRandomToken () + ".scriptlet")
		if _descriptor, _error := syscall.Open (_temporaryPath, syscall.O_CREAT | syscall.O_EXCL | syscall.O_WRONLY, 0600); _error == nil {
			...
			return nil, nil, errorw (0x2b19feaa, _error)
		}
